...
const questionParser = (qset: Question[], updateResponseText: any) => {
  // This function takes a question and returns component specific to that question type
  let formCreater: any[] = [];
  qset.forEach((q: Question) => {
    if (q.type === "text") {
      formCreater.push(TextComponent(q, updateResponseText));
    } else if (q.type === "longText") {
      formCreater.push(LongTextComponent(q, updateResponseText));
